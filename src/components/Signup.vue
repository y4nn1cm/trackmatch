    ...
    <v-flex xs12 sm6 offset-sm3 mt-3>
      <form @submit.prevent="userSignUp">
        <v-layout column>
          ...
          <v-flex>
            <v-text-field name="firstname" label="Firstname" id="firstname" type="text" v-model="firstname" required></v-text-field>
          </v-flex>
          <v-flex>
            <v-text-field name="lastname" label="Lastname" id="lastname" type="text" v-model="lastname" required></v-text-field>
          </v-flex>
          <v-flex>
            <v-text-field name="password" label="Password" id="password" type="password" v-model="password" required></v-text-field>
          </v-flex>
          <v-flex>
            <v-text-field name="confirmPassword" label="Confirm Password" id="confirmPassword" type="password" v-model="passwordConfirm" :rules="[comparePasswords]"></v-text-field>
          </v-flex>
          <v-flex>
            <v-radio style="margin-bottom:0; padding:0" label="I am exploring job opportunities" value="jobs" v-model="reason"></v-radio>
          </v-flex>
          <v-flex>
